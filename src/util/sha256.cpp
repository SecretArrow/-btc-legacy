// src/util/sha256.cpp
//
// SHA-256 reference implementation. Verified against FIPS 180-4 test
// vectors for empty input, "abc", and the 448-bit test message.
//
#include "btclegacy/util/sha256.h"
#include <cstring>
#include <cstdint>
#include <cstdio>

namespace btclegacy::util {

namespace {

constexpr uint32_t K[64] = {
    0x428a2f98u, 0x71374491u, 0xb5c0fbcfu, 0xe9b5dba5u,
    0x3956c25bu, 0x59f111f1u, 0x923f82a4u, 0xab1c5ed5u,
    0xd807aa98u, 0x12835b01u, 0x243185beu, 0x550c7dc3u,
    0x72be5d74u, 0x80deb1feu, 0x9bdc06a7u, 0xc19bf174u,
    0xe49b69c1u, 0xefbe4786u, 0x0fc19dc6u, 0x240ca1ccu,
    0x2de92c6fu, 0x4a7484aau, 0x5cb0a9dcu, 0x76f988dau,
    0x983e5152u, 0xa831c66du, 0xb00327c8u, 0xbf597fc7u,
    0xc6e00bf3u, 0xd5a79147u, 0x06ca6351u, 0x14292967u,
    0x27b70a85u, 0x2e1b2138u, 0x4d2c6dfcu, 0x53380d13u,
    0x650a7354u, 0x766a0abbu, 0x81c2c92eu, 0x92722c85u,
    0xa2bfe8a1u, 0xa81a664bu, 0xc24b8b70u, 0xc76c51a3u,
    0xd192e819u, 0xd6990624u, 0xf40e3585u, 0x106aa070u,
    0x19a4c116u, 0x1e376c08u, 0x2748774cu, 0x34b0bcb5u,
    0x391c0cb3u, 0x4ed8aa4au, 0x5b9cca4fu, 0x682e6ff3u,
    0x748f82eeu, 0x78a5636fu, 0x84c87814u, 0x8cc70208u,
    0x90befffau, 0xa4506cebu, 0xbef9a3f7u, 0xc67178f2u,
};

constexpr uint32_t IV[8] = {
    0x6a09e667u, 0xbb67ae85u, 0x3c6ef372u, 0xa54ff53au,
    0x510e527fu, 0x9b05688cu, 0x1f83d9abu, 0x5be0cd19u,
};

inline uint32_t rotr(uint32_t x, uint32_t n) {
    return (x >> n) | (x << (32 - n));
}

inline void sha256_block(uint32_t h[8], const uint8_t* p) {
    uint32_t w[64];
    for (int i = 0; i < 16; ++i) {
        w[i] = (uint32_t(p[i*4])   << 24) |
               (uint32_t(p[i*4+1]) << 16) |
               (uint32_t(p[i*4+2]) <<  8) |
               (uint32_t(p[i*4+3]));
    }
    for (int i = 16; i < 64; ++i) {
        uint32_t s0 = rotr(w[i-15], 7) ^ rotr(w[i-15], 18) ^ (w[i-15] >> 3);
        uint32_t s1 = rotr(w[i-2], 17) ^ rotr(w[i-2], 19) ^ (w[i-2] >> 10);
        w[i] = w[i-16] + s0 + w[i-7] + s1;
    }
    uint32_t a=h[0],b=h[1],c=h[2],d=h[3],e=h[4],f=h[5],g=h[6],hh=h[7];
    for (int i = 0; i < 64; ++i) {
        uint32_t S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
        uint32_t ch = (e & f) ^ ((~e) & g);
        uint32_t t1 = hh + S1 + ch + K[i] + w[i];
        uint32_t S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
        uint32_t mj = (a & b) ^ (a & c) ^ (b & c);
        uint32_t t2 = S0 + mj;
        hh = g;
        g  = f;
        f  = e;
        e  = d + t1;
        d  = c;
        c  = b;
        b  = a;
        a  = t1 + t2;
    }
    h[0]+=a; h[1]+=b; h[2]+=c; h[3]+=d; h[4]+=e; h[5]+=f; h[6]+=g; h[7]+=hh;
}
} // namespace

Sha256::Sha256() { reset(); }

void Sha256::reset() {
    for (int i = 0; i < 8; ++i) h_[i] = IV[i];
    total_ = 0;
    buflen_ = 0;
}

void Sha256::update(const uint8_t* data, size_t len) {
    total_ += len;
    while (len > 0) {
        size_t take = 64 - buflen_;
        if (take > len) take = len;
        std::memcpy(buf_ + buflen_, data, take);
        buflen_ += take;
        data += take;
        len  -= take;
        if (buflen_ == 64) {
            sha256_block(h_, buf_);
            buflen_ = 0;
        }
    }
}

void Sha256::update(const std::string& s) {
    update(reinterpret_cast<const uint8_t*>(s.data()), s.size());
}

void Sha256::update(const std::vector<uint8_t>& v) {
    update(v.data(), v.size());
}

void Sha256::finish(uint8_t out[32]) {
    // Append the 0x80 byte after the message data.
    buf_[buflen_++] = 0x80;
    // If we don't have room for the 8-byte length suffix in this block,
    // pad with zeros to the end, process the block, then continue in
    // a fresh second block.
    if (buflen_ > 56) {
        while (buflen_ < 64) buf_[buflen_++] = 0;
        sha256_block(h_, buf_);
        buflen_ = 0;
    }
    // Pad with zeros up to the length-field position.
    while (buflen_ < 56) buf_[buflen_++] = 0;
    // Append 64-bit big-endian length in bits.
    uint64_t bits = total_ * 8;
    for (int i = 7; i >= 0; --i) {
        buf_[buflen_++] = uint8_t((bits >> (i*8)) & 0xff);
    }
    sha256_block(h_, buf_);
    // Emit hash bytes in big-endian order.
    for (int i = 0; i < 8; ++i) {
        out[i*4+0] = uint8_t((h_[i] >> 24) & 0xff);
        out[i*4+1] = uint8_t((h_[i] >> 16) & 0xff);
        out[i*4+2] = uint8_t((h_[i] >>  8) & 0xff);
        out[i*4+3] = uint8_t((h_[i]      ) & 0xff);
    }
    reset();
}

static const char* hexdig = "0123456789abcdef";
std::string to_hex_helper(const uint8_t* data, size_t len) {
    std::string s; s.reserve(len * 2);
    for (size_t i = 0; i < len; ++i) {
        s.push_back(hexdig[data[i] >> 4]);
        s.push_back(hexdig[data[i] & 0x0f]);
    }
    return s;
}

std::string sha256_hex(const uint8_t* data, size_t len) {
    Sha256 s; s.update(data, len);
    uint8_t out[32]; s.finish(out);
    return to_hex_helper(out, 32);
}

std::string sha256_hex_file(const std::string& path) {
    FILE* f = std::fopen(path.c_str(), "rb");
    if (!f) return "";
    Sha256 s; uint8_t buf[65536]; size_t n;
    while ((n = std::fread(buf, 1, sizeof(buf), f)) > 0) s.update(buf, n);
    std::fclose(f);
    uint8_t out[32]; s.finish(out);
    return to_hex_helper(out, 32);
}

} // namespace btclegacy::util
